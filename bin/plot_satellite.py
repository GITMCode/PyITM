...
def makesatplot(satDataDict, savepath, verbose=False, saveName=None):
    ...
    didSmooth = 'smoothed_sat_density' in satDataDict.keys() 
    
    if didSmooth:
        axs[0].plot(satDataDict['times'], satDataDict['model_rho'], 'b', 
                    alpha = 0.1)
        axs[0].plot(satDataDict['times'], satDataDict['sat_density'], 'r', 
                    alpha = 0.1)
        axs[0].plot(satDataDict['times'], satDataDict['smoothed_model_rho'], 'b', 
                    label="Modeled Density")
        axs[0].plot(satDataDict['times'], satDataDict['smoothed_sat_density'], 'r', 
    ...
    if didSmooth:
        axs[1].plot(satDataDict['times'], 
                    100*(satDataDict['sat_density']- satDataDict['model_rho'])
                    /satDataDict['sat_density'],
                    alpha = 0.1, color='k')
        axs[1].plot(satDataDict['times'], 
                    100*(satDataDict['smoothed_sat_density']-satDataDict['smoothed_model_rho'])
                    /satDataDict['smoothed_sat_density'],
                    color='k')
    else:
        axs[1].plot(satDataDict['times'], 
                    100*(satDataDict['sat_density']- satDataDict['model_rho'])
    ...
    fig.suptitle(satDataDict['sat_name'])
